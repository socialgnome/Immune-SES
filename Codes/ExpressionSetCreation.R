#### This file is a script that is responsible for creating an expression set object for mediation analysis and WGCNA
...
## Notes ##
# Voom correcting the count data for mediation is new. The PNAS paper did not voom
# correct, instead we took the counts from normalization and batch corrected.
# However, to keep things consistent with the GRN analysis, we are voom correcting
# here. To see batch correction without voom, refer to the PNAS expression set 
# creation codes.


## Save Count data for mediation
phenoData <- new("AnnotatedDataFrame", data=subData)
filtexprs <- ExpressionSet(assayData=v$E,phenoData=phenoData)
